...
cursor.execute('''CREATE TABLE IF NOT EXISTS users (
    user_id INTEGER PRIMARY KEY, balance INTEGER DEFAULT 0, referrals INTEGER DEFAULT 0, referred_by INTEGER
)''')
conn.commit()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        if ref_id != user_id:  # Не самоприглашение
            ...
            cursor.execute('UPDATE users SET referrals = referrals + 1, balance = balance + 10 WHERE user_id=?', (ref_id,))
            conn.commit()
            # Крутое уведомление рефереру
    ...
    # Крутой текст для /start
    text = f"""🚀 *Добро пожаловать в рефералку!*

💰 *Твой баланс:* {data[0]} монет | 👥 *Рефералов:* {data[1]}
...
👇 *Выбери действие:*"""
    ...
    keyboard = [[InlineKeyboardButton("💎 Моя ссылка", callback_data='link'),
...
📈 *Уже {refs} человек в твоей сети!*
*Поделись сейчас → стань топ-1!* 👑"""
        
        await query.edit_message_text(text, parse_mode='Markdown')
        
    elif query.data == 'leaderboard':
        cursor.execute('SELECT user_id, referrals, balance FROM users ORDER BY referrals DESC LIMIT 10')
        top = cursor.fetchall()
        ...
        cursor.execute('SELECT referrals FROM users WHERE user_id=?', (user_id,))
        user_refs = cursor.fetchone()[0] or 0
